				   ...
				   PSTR cmdLine, int showCmd)
{
	...
	int w(800), h(600);
	if (!WinUtil::Get().InitMainWindow(w, h, hInstance, "Aguraki", MainWndProc, true))
		assert(false);
	...
	DirectX::SpriteFont *mpFont = new SpriteFont(&d3d.GetDevice(), L"data/fonts/comicSansMS.spritefont");
	Game game(d3d, mpFont);
	...
	while (WinUtil::Get().BeginLoop(canUpdateRender))
	{
		...
		{
			game.Update(dTime, game.startOfStage);
			game.Render(dTime);
		}
